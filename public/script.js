...
function startCountdown() {
  ...
  countdownInterval = setInterval(() => {
    ...
    if (countdown === 0) {
      ...
      socket.emit('countdownFinished', room);
    }
  }, 1000);
}
...
socket.on('roomCreated', ({ roomName, player: p }) => {
  ...
  document.getElementById('gameCanvas').style.display = 'block';
  document.querySelectorAll('.controles-lateral').forEach(el => el.style.display = 'block');
  socket.emit('playerReady', roomName);
});

socket.on('roomReady', () => socket.emit('playerReady', room));
...
socket.on('showGameOver', (winner) => {
  ...
  document.getElementById('winnerText').innerText = winner === 1 ? "¡Jugador 1 gana!" : "¡Jugador 2 gana!";
  if (isHost) {
    document.getElementById('startBtn').style.display = 'none';
  ...
  document.querySelectorAll('.controles-lateral').forEach(el => el.style.display = 'none');
});
